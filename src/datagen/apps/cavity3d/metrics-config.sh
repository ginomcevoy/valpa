...
# metrics-config.sh
# Creates metrics-app.csv from times.txt, std.out and custom.out 
# for all executions of a single cluster configuration
# Works on the Palabos cavity3d benchmark
...
python $LOCAL_DIR/cavity3d-output-reader.py $CONFIG_DIR/times.txt $OUTPUT_FILE $EXEC_DIRS
status=$?
if [ $status -ne 0 ]; then
    echo "ERROR: Faulty data for config $CONFIG_DIR"
fi
