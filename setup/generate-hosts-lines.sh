...
# Output file
OUTPUT="$VESPA_DIR/data-output/hosts"
rm $OUTPUT
...
do
	# Node name
	NODE_NAME=$($VESPA_DIR/util/iterator-node-names.sh $i)

    # First node will get $IP_PREFIX.1
	IP_ADDRESS=$($VESPA_DIR/networking/get-node-address.sh $i)
...
do
	...
	# Node number
	NODE_NUMBER=$($VESPA_DIR/util/iterator-node-numbers.sh $NODE_INDEX)
	...
	do
		# VM name
		VM_NUMBER=$(expr $vm + 1)
		VM_NAME=$($VESPA_DIR/util/iterator-vm-names.sh $NODE_INDEX $vm)
		VM_ADDRESS=$($VESPA_DIR/networking/get-vm-address.sh $NODE_INDEX $vm)
